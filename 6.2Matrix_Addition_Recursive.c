#include <stdio.h>
#include <stdlib.h>
void add_matrices(int **A, int**B, int **C, int rows, int cols, int i, int j);
int main()
{
...
void add_matrices(int **A, int**B, int **C, int rows, int cols, int i, int j)
{
    ...
    if(j==cols)
    {
        add_matrices(A, B, C, rows, cols, i+1, 0);
        return;
    }
    C[i][j]=A[i][j]+B[i][j];
    add_matrices(A, B, C, rows, cols, i, j+1);
}
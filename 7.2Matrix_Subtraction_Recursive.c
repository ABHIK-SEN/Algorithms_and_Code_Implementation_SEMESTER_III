#include <stdio.h>
#include <stdlib.h>
void sub_matrices(int **A, int**B, int **C, int rows, int cols, int i, int j);
int main()
{
...
void sub_matrices(int **A, int**B, int **C, int rows, int cols, int i, int j)
{
    ...
    {
        C[i][j]=A[i][j]-B[i][j];
        sub_matrices(A, B, C, rows, cols, i, j+1);
    }
    else
    {
        sub_matrices(A, B, C, rows, cols, i+1, 0);
        return;
    }
}